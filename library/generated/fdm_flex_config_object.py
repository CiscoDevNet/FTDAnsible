...
DOCUMENTATION = """
---
module: fdm_flex_config_object
short_description: Manages FlexConfigObject objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class FlexConfigObjectResource(object):
    ...
    def addFlexConfigObject(params):
        body_params = dict_subset(params, ['version', 'name', 'description', 'lines', 'negateLines', 'variables', 'id', 'type'])
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteFlexConfigObject(params):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getFlexConfigObject(params):
        path_params = dict_subset(params, ['objId'])
    ...
    @staticmethod
    @retry_on_token_expiration
    def getFlexConfigObjectList(params):
        query_params = dict_subset(params, ['offset', 'limit', 'sort', 'filter'])

        url = construct_url(params['hostname'], '/object/flexconfigobjects', query_params=query_params)
        request_params = dict(
            headers=base_headers(params['access_token']),
            method='GET',
        )
    ...
    @staticmethod
    @retry_on_token_expiration
    def getFlexConfigObjectByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(FlexConfigObjectResource.getFlexConfigObjectList, search_params)
    ...
    def upsertFlexConfigObject(params):
        ...
        try:
            ...
            if is_duplicate_name_error(e):
                existing_object = FlexConfigObjectResource.getFlexConfigObjectByName(params)
                params = FlexConfigObjectResource.copy_identity_params(existing_object, params)
                return FlexConfigObjectResource.editFlexConfigObject(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editFlexConfigObjectByName(params):
        existing_object = FlexConfigObjectResource.getFlexConfigObjectByName(params)
        params = FlexConfigObjectResource.copy_identity_params(existing_object, params)
        return FlexConfigObjectResource.editFlexConfigObject(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteFlexConfigObjectByName(params):
        existing_object = FlexConfigObjectResource.getFlexConfigObjectByName(params)
        params = FlexConfigObjectResource.copy_identity_params(existing_object, params)
        return FlexConfigObjectResource.deleteFlexConfigObject(params)
...
def main():
    ...
    try:
        method_to_call = getattr(FlexConfigObjectResource, params['operation'])
