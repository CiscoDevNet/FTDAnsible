...
DOCUMENTATION = """
---
module: fdm_device_dns_settings
short_description: Manages DeviceDNSSettings objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class DeviceDNSSettingsResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getDeviceDNSSettingsByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(DeviceDNSSettingsResource.getDeviceDNSSettingsList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def editDeviceDNSSettingsByName(params):
        existing_object = DeviceDNSSettingsResource.getDeviceDNSSettingsByName(params)
        params = DeviceDNSSettingsResource.copy_identity_params(existing_object, params)
        return DeviceDNSSettingsResource.editDeviceDNSSettings(params)
...
def main():
    ...
    try:
        method_to_call = getattr(DeviceDNSSettingsResource, params['operation'])
