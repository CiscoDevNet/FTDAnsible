...
DOCUMENTATION = """
---
module: ftd_telemetry
short_description: Manages Telemetry objects on Cisco FTD devices
version_added: "2.7"
...
class TelemetryResource(object):
    
    @staticmethod
    @retry_on_token_expiration
    def getTelemetry(params):
        path_params = dict_subset(params, ['objId'])

        url = construct_url(params['hostname'], '/operational/telemetry/{objId}', path_params=path_params)
        request_params = dict(
...
def main():
    fields = dict(
        ...
        operation=dict(type='str', choices=['getTelemetry'], required=True),
    ...
    try:
        method_to_call = getattr(TelemetryResource, params['operation'])
