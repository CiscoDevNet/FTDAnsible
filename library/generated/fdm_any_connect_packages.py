...
DOCUMENTATION = """
---
module: fdm_any_connect_packages
short_description: Manages AnyConnectPackages objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class AnyConnectPackagesResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteAnyConnectPackages(params):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getAnyConnectPackages(params):
        path_params = dict_subset(params, ['objId'])
    ...
    @staticmethod
    @retry_on_token_expiration
    def getAnyConnectPackagesByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(AnyConnectPackagesResource.getAnyConnectPackagesList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertAnyConnectPackages(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return AnyConnectPackagesResource.addAnyConnectPackages(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = AnyConnectPackagesResource.getAnyConnectPackagesByName(params)
                params = AnyConnectPackagesResource.copy_identity_params(existing_object, params)
                return AnyConnectPackagesResource.editAnyConnectPackages(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editAnyConnectPackagesByName(params):
        existing_object = AnyConnectPackagesResource.getAnyConnectPackagesByName(params)
        params = AnyConnectPackagesResource.copy_identity_params(existing_object, params)
        return AnyConnectPackagesResource.editAnyConnectPackages(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteAnyConnectPackagesByName(params):
        existing_object = AnyConnectPackagesResource.getAnyConnectPackagesByName(params)
        params = AnyConnectPackagesResource.copy_identity_params(existing_object, params)
        return AnyConnectPackagesResource.deleteAnyConnectPackages(params)
...
def main():
    ...
    try:
        method_to_call = getattr(AnyConnectPackagesResource, params['operation'])
