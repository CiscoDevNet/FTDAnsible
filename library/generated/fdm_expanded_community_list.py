...
DOCUMENTATION = """
---
module: fdm_expanded_community_list
short_description: Manages ExpandedCommunityList objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class ExpandedCommunityListResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteExpandedCommunityList(params):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getExpandedCommunityList(params):
        path_params = dict_subset(params, ['objId'])
    ...
    @staticmethod
    @retry_on_token_expiration
    def getExpandedCommunityListByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(ExpandedCommunityListResource.getExpandedCommunityListList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertExpandedCommunityList(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return ExpandedCommunityListResource.addExpandedCommunityList(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = ExpandedCommunityListResource.getExpandedCommunityListByName(params)
                params = copy_identity_properties(existing_object, params)
                return ExpandedCommunityListResource.editExpandedCommunityList(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editExpandedCommunityListByName(params):
        existing_object = ExpandedCommunityListResource.getExpandedCommunityListByName(params)
        params = copy_identity_properties(existing_object, params)
        return ExpandedCommunityListResource.editExpandedCommunityList(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteExpandedCommunityListByName(params):
        existing_object = ExpandedCommunityListResource.getExpandedCommunityListByName(params)
        params = copy_identity_properties(existing_object, params)
        return ExpandedCommunityListResource.deleteExpandedCommunityList(params)
...
def main():
    fields = dict(
        ...
        operation=dict(choices=['addExpandedCommunityList', 'deleteExpandedCommunityList', 'editExpandedCommunityList', 'getExpandedCommunityList', 'getExpandedCommunityListList', 'getExpandedCommunityListByName', 'upsertExpandedCommunityList', 'editExpandedCommunityListByName', 'deleteExpandedCommunityListByName'], required=True),
    ...
    try:
        method_to_call = getattr(ExpandedCommunityListResource, params['operation'])
