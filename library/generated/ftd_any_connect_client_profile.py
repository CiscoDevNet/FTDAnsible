...
options:
  ...
  md5Checksum
    description:
      - A string containing the MD5 checksum of the client profile XML file. The system generates the checksum when the file is uploaded.<br>Field level constraints: must match pattern ^((?!;).)*$. (Note: Additional constraints might exist)
  name
    description:
...
class AnyConnectClientProfileResource(object):
    ...
    def addAnyConnectClientProfile(self, params):
        body_params = dict_subset(params, ['description', 'diskFileName', 'id', 'md5Checksum', 'name', 'type', 'version'])

        return self._conn.send_request(
            url_path='/object/anyconnectclientprofiles',
            http_method='POST',
            body_params=body_params,
        )

    def deleteAnyConnectClientProfile(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/object/anyconnectclientprofiles/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editAnyConnectClientProfile(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['description', 'diskFileName', 'id', 'md5Checksum', 'name', 'type', 'version'])
    ...
    def getAnyConnectClientProfile(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getAnyConnectClientProfileList(self, params):
    ...
    def getAnyConnectClientProfileByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getAnyConnectClientProfileList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertAnyConnectClientProfile(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addAnyConnectClientProfile(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getAnyConnectClientProfileByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editAnyConnectClientProfile(params)
            else:
                raise e

    def editAnyConnectClientProfileByName(self, params):
        existing_object = self.getAnyConnectClientProfileByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editAnyConnectClientProfile(params)

    def deleteAnyConnectClientProfileByName(self, params):
        existing_object = self.getAnyConnectClientProfileByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteAnyConnectClientProfile(params)
...
def main():
    ...
    try:
        ...
        resource = AnyConnectClientProfileResource(conn)
