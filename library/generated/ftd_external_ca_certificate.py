...
class ExternalCACertificateResource(object):
    ...
    def addExternalCACertificate(self, params):
        ...
        return self._conn.send_request(
            url_path='/object/externalcacertificates',
            http_method='POST',
            body_params=body_params,
        )

    def deleteExternalCACertificate(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/object/externalcacertificates/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editExternalCACertificate(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['cert', 'id', 'issuerCommonName', 'issuerCountry', 'issuerLocality', 'issuerOrganization', 'issuerOrganizationUnit', 'issuerState', 'isSystemDefined', 'name', 'passPhrase', 'privateKey', 'subjectCommonName', 'subjectCountry', 'subjectDistinguishedName', 'subjectLocality', 'subjectOrganization', 'subjectOrganizationUnit', 'subjectState', 'type', 'validityEndDate', 'validityStartDate', 'version'])
    ...
    def getExternalCACertificate(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getExternalCACertificateList(self, params):
    ...
    def getExternalCACertificateByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getExternalCACertificateList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertExternalCACertificate(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addExternalCACertificate(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getExternalCACertificateByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editExternalCACertificate(params)
            else:
                raise e

    def editExternalCACertificateByName(self, params):
        existing_object = self.getExternalCACertificateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editExternalCACertificate(params)

    def deleteExternalCACertificateByName(self, params):
        existing_object = self.getExternalCACertificateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteExternalCACertificate(params)
...
def main():
    ...
    try:
        ...
        resource = ExternalCACertificateResource(conn)
