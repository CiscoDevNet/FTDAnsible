...
class GeolocationUpdateScheduleResource(object):
    ...
    def addGeolocationUpdateSchedule(self, params):
        ...
        return self._conn.send_request(
            url_path='/managedentity/geolocationupdateschedules',
            http_method='POST',
    ...
    def editGeolocationUpdateSchedule(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['description', 'forceOperation', 'id', 'jobHistoryUuid', 'jobName', 'name', 'runTimes', 'scheduleType', 'type', 'user', 'version'])
    ...
    def getGeolocationUpdateSchedule(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getGeolocationUpdateScheduleList(self, params):
    ...
    def getGeolocationUpdateScheduleByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getGeolocationUpdateScheduleList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertGeolocationUpdateSchedule(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addGeolocationUpdateSchedule(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getGeolocationUpdateScheduleByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editGeolocationUpdateSchedule(params)
            else:
                raise e

    def editGeolocationUpdateScheduleByName(self, params):
        existing_object = self.getGeolocationUpdateScheduleByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editGeolocationUpdateSchedule(params)

    def deleteGeolocationUpdateScheduleByName(self, params):
        existing_object = self.getGeolocationUpdateScheduleByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteGeolocationUpdateSchedule(params)
...
def main():
    ...
    try:
        ...
        resource = GeolocationUpdateScheduleResource(conn)
