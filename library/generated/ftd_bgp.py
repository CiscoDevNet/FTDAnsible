...
class BGPResource(object):
    ...
    def addBGP(params):
        ...
        url = construct_url(params['hostname'], '/devices/default/routing/virtualrouters/default/bgp')
        request_params = dict(
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteBGP(params):
        path_params = dict_subset(params, ['objId'])

        url = construct_url(params['hostname'], '/devices/default/routing/virtualrouters/default/bgp/{objId}', path_params=path_params)
        request_params = dict(
            headers=base_headers(params['access_token']),
            method='DELETE',
        )
    ...
    @staticmethod
    @retry_on_token_expiration
    def getBGPList(params):
        query_params = dict_subset(params, ['filter', 'limit', 'offset', 'sort'])

        url = construct_url(params['hostname'], '/devices/default/routing/virtualrouters/default/bgp', query_params=query_params)
        request_params = dict(
            headers=base_headers(params['access_token']),
            method='GET',
        )
    ...
    @staticmethod
    @retry_on_token_expiration
    def getBGPByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(BGPResource.getBGPList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertBGP(params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return BGPResource.addBGP(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = BGPResource.getBGPByName(params)
                params = copy_identity_properties(existing_object, params)
                return BGPResource.editBGP(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editBGPByName(params):
        existing_object = BGPResource.getBGPByName(params)
        params = copy_identity_properties(existing_object, params)
        return BGPResource.editBGP(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteBGPByName(params):
        existing_object = BGPResource.getBGPByName(params)
        params = copy_identity_properties(existing_object, params)
        return BGPResource.deleteBGP(params)
...
def main():
    ...
    try:
        method_to_call = getattr(BGPResource, params['operation'])
