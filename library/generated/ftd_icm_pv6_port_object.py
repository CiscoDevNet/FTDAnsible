...
DOCUMENTATION = """
---
module: ftd_icm_pv6_port_object
short_description: Manages ICMPv6PortObject objects on Cisco FTD devices
version_added: "2.7"
...
EXAMPLES = """
- name: Fetch ICMPv6PortObject with a given name
...
class ICMPv6PortObjectResource(object):

    def __init__(self, conn):
        self._conn = conn

    def addICMPv6PortObject(self, params):
        body_params = dict_subset(params, ['description', 'icmpv6Code', 'icmpv6Type', 'id', 'isSystemDefined', 'name', 'type', 'version'])

        return self._conn.send_request(
            url_path='/object/icmpv6ports',
            http_method='POST',
            body_params=body_params,
        )

    def deleteICMPv6PortObject(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/object/icmpv6ports/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editICMPv6PortObject(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['description', 'icmpv6Code', 'icmpv6Type', 'id', 'isSystemDefined', 'name', 'type', 'version'])
    ...
    def getICMPv6PortObject(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getICMPv6PortObjectList(self, params):
    ...
    def getICMPv6PortObjectByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getICMPv6PortObjectList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertICMPv6PortObject(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addICMPv6PortObject(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getICMPv6PortObjectByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editICMPv6PortObject(params)
            else:
                raise e

    def editICMPv6PortObjectByName(self, params):
        existing_object = self.getICMPv6PortObjectByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editICMPv6PortObject(params)

    def deleteICMPv6PortObjectByName(self, params):
        existing_object = self.getICMPv6PortObjectByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteICMPv6PortObject(params)
...
def main():
    ...
    try:
        ...
        resource = ICMPv6PortObjectResource(conn)
