...
class IkevOnePolicyResource(object):
    ...
    def addIkevOnePolicy(self, params):
        ...
        return self._conn.send_request(
            url_path='/object/ikev1policies',
            http_method='POST',
            body_params=body_params,
        )

    def deleteIkevOnePolicy(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/object/ikev1policies/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editIkevOnePolicy(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['authenticationType', 'cryptoRestricted', 'enabled', 'encryptionType', 'groupType', 'hashType', 'id', 'isSystemDefined', 'lifeTime', 'name', 'priority', 'summaryLabel', 'type', 'version'])
    ...
    def getIkevOnePolicy(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getIkevOnePolicyList(self, params):
    ...
    def getIkevOnePolicyByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getIkevOnePolicyList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertIkevOnePolicy(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addIkevOnePolicy(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getIkevOnePolicyByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editIkevOnePolicy(params)
            else:
                raise e

    def editIkevOnePolicyByName(self, params):
        existing_object = self.getIkevOnePolicyByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editIkevOnePolicy(params)

    def deleteIkevOnePolicyByName(self, params):
        existing_object = self.getIkevOnePolicyByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteIkevOnePolicy(params)
...
def main():
    ...
    try:
        ...
        resource = IkevOnePolicyResource(conn)
