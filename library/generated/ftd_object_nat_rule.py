...
class ObjectNatRuleResource(object):
    ...
    def addObjectNatRule(self, params):
        ...
        return self._conn.send_request(
            url_path='/policy/objectnatpolicies/{parentId}/objectnatrules',
            http_method='POST',
    ...
    def deleteObjectNatRule(self, params):
        path_params = dict_subset(params, ['objId', 'parentId'])

        return self._conn.send_request(
            url_path='/policy/objectnatpolicies/{parentId}/objectnatrules/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editObjectNatRule(self, params):
        path_params = dict_subset(params, ['objId', 'parentId'])
        query_params = dict_subset(params, ['at'])
        body_params = dict_subset(params, ['description', 'destinationInterface', 'dns', 'enabled', 'id', 'interfaceInTranslatedNetwork', 'interfaceIPv6', 'name', 'natType', 'netToNet', 'noProxyArp', 'originalNetwork', 'originalPort', 'patOptions', 'routeLookup', 'sourceInterface', 'translatedNetwork', 'translatedPort', 'type', 'version'])
    ...
    def getObjectNatRule(self, params):
        path_params = dict_subset(params, ['objId', 'parentId'])
    ...
    def getObjectNatRuleList(self, params):
        path_params = dict_subset(params, ['parentId'])
    ...
    def getObjectNatRuleByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getObjectNatRuleList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertObjectNatRule(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addObjectNatRule(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getObjectNatRuleByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editObjectNatRule(params)
            else:
                raise e

    def editObjectNatRuleByName(self, params):
        existing_object = self.getObjectNatRuleByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editObjectNatRule(params)

    def deleteObjectNatRuleByName(self, params):
        existing_object = self.getObjectNatRuleByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteObjectNatRule(params)
...
def main():
    ...
    try:
        ...
        resource = ObjectNatRuleResource(conn)
