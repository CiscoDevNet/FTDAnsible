...
class ProtocolObjectResource(object):
    ...
    def addProtocolObject(self, params):
        body_params = dict_subset(params, ['description', 'id', 'isSystemDefined', 'name', 'protocol', 'type', 'version'])

        return self._conn.send_request(
            url_path='/object/protocols',
            http_method='POST',
            body_params=body_params,
        )

    def deleteProtocolObject(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/object/protocols/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editProtocolObject(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['description', 'id', 'isSystemDefined', 'name', 'protocol', 'type', 'version'])
    ...
    def getProtocolObject(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getProtocolObjectList(self, params):
    ...
    def getProtocolObjectByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getProtocolObjectList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertProtocolObject(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addProtocolObject(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getProtocolObjectByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editProtocolObject(params)
            else:
                raise e

    def editProtocolObjectByName(self, params):
        existing_object = self.getProtocolObjectByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editProtocolObject(params)

    def deleteProtocolObjectByName(self, params):
        existing_object = self.getProtocolObjectByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteProtocolObject(params)
...
def main():
    ...
    try:
        ...
        resource = ProtocolObjectResource(conn)
