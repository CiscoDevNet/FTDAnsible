...
class DNSServerGroupResource(object):
    ...
    def addDNSServerGroup(self, params):
        body_params = dict_subset(params, ['dnsServers', 'id', 'name', 'retries', 'searchDomain', 'systemDefined', 'timeout', 'type', 'version'])

        return self._conn.send_request(
            url_path='/object/dnsservergroups',
            http_method='POST',
            body_params=body_params,
        )

    def deleteDNSServerGroup(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/object/dnsservergroups/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editDNSServerGroup(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['dnsServers', 'id', 'name', 'retries', 'searchDomain', 'systemDefined', 'timeout', 'type', 'version'])
    ...
    def getDNSServerGroup(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getDNSServerGroupList(self, params):
    ...
    def getDNSServerGroupByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getDNSServerGroupList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertDNSServerGroup(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addDNSServerGroup(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getDNSServerGroupByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editDNSServerGroup(params)
            else:
                raise e

    def editDNSServerGroupByName(self, params):
        existing_object = self.getDNSServerGroupByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editDNSServerGroup(params)

    def deleteDNSServerGroupByName(self, params):
        existing_object = self.getDNSServerGroupByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteDNSServerGroup(params)
...
def main():
    ...
    try:
        ...
        resource = DNSServerGroupResource(conn)
