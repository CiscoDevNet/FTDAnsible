...
class IntrusionSettingsResource(object):
    ...
    def editIntrusionSettings(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['id', 'name', 'syslogServer', 'type', 'version'])

        return self._conn.send_request(
            url_path='/object/intrusionsettings/{objId}',
            http_method='PUT',
            body_params=body_params,
            path_params=path_params,
        )

    def getIntrusionSettings(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getIntrusionSettingsList(self, params):
        query_params = dict_subset(params, ['filter', 'limit', 'offset', 'sort'])

        return self._conn.send_request(
            url_path='/object/intrusionsettings',
            http_method='GET',
            query_params=query_params,
        )

    def getIntrusionSettingsByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getIntrusionSettingsList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def editIntrusionSettingsByName(self, params):
        existing_object = self.getIntrusionSettingsByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editIntrusionSettings(params)
...
def main():
    ...
    try:
        ...
        resource = IntrusionSettingsResource(conn)
