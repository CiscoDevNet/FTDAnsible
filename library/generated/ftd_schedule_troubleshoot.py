...
class ScheduleTroubleshootResource(object):
    ...
    def addScheduleTroubleshoot(self, params):
        body_params = dict_subset(params, ['forceOperation', 'id', 'ipAddress', 'jobHistoryUuid', 'jobName', 'scheduleType', 'type', 'user', 'version'])

        return self._conn.send_request(
            url_path='/action/troubleshoot',
            http_method='POST',
            body_params=body_params,
        )

    def deleteScheduleTroubleshoot(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/action/troubleshoot/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editScheduleTroubleshoot(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['forceOperation', 'id', 'ipAddress', 'jobHistoryUuid', 'jobName', 'scheduleType', 'type', 'user', 'version'])
    ...
    def getScheduleTroubleshoot(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getScheduleTroubleshootList(self, params):
    ...
    def getScheduleTroubleshootByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getScheduleTroubleshootList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertScheduleTroubleshoot(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addScheduleTroubleshoot(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getScheduleTroubleshootByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editScheduleTroubleshoot(params)
            else:
                raise e

    def editScheduleTroubleshootByName(self, params):
        existing_object = self.getScheduleTroubleshootByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editScheduleTroubleshoot(params)

    def deleteScheduleTroubleshootByName(self, params):
        existing_object = self.getScheduleTroubleshootByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteScheduleTroubleshoot(params)
...
def main():
    ...
    try:
        ...
        resource = ScheduleTroubleshootResource(conn)
