...
DOCUMENTATION = """
---
module: fdm_backup_scheduled
short_description: Manages BackupScheduled objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class BackupScheduledResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteBackupScheduled(params):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getBackupScheduledByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(BackupScheduledResource.getBackupScheduledList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertBackupScheduled(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return BackupScheduledResource.addBackupScheduled(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = BackupScheduledResource.getBackupScheduledByName(params)
                params = BackupScheduledResource.copy_identity_params(existing_object, params)
                return BackupScheduledResource.editBackupScheduled(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editBackupScheduledByName(params):
        existing_object = BackupScheduledResource.getBackupScheduledByName(params)
        params = BackupScheduledResource.copy_identity_params(existing_object, params)
        return BackupScheduledResource.editBackupScheduled(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteBackupScheduledByName(params):
        existing_object = BackupScheduledResource.getBackupScheduledByName(params)
        params = BackupScheduledResource.copy_identity_params(existing_object, params)
        return BackupScheduledResource.deleteBackupScheduled(params)
...
def main():
    ...
    try:
        method_to_call = getattr(BackupScheduledResource, params['operation'])
