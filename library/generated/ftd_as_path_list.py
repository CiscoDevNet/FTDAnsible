...
class ASPathListResource(object):
    ...
    def addASPathList(self, params):
        body_params = dict_subset(params, ['description', 'entries', 'id', 'name', 'type', 'version'])

        return self._conn.send_request(
            url_path='/object/aspathlists',
            http_method='POST',
            body_params=body_params,
        )

    def deleteASPathList(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/object/aspathlists/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editASPathList(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['description', 'entries', 'id', 'name', 'type', 'version'])
    ...
    def getASPathListList(self, params):
    ...
    def getASPathListByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getASPathListList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertASPathList(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addASPathList(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getASPathListByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editASPathList(params)
            else:
                raise e

    def editASPathListByName(self, params):
        existing_object = self.getASPathListByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editASPathList(params)

    def deleteASPathListByName(self, params):
        existing_object = self.getASPathListByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteASPathList(params)
...
def main():
    ...
    try:
        ...
        resource = ASPathListResource(conn)
