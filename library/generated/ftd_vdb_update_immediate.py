...
class VDBUpdateImmediateResource(object):
    ...
    def addVDBUpdateImmediate(self, params):
        ...
        return self._conn.send_request(
            url_path='/action/updatevdb',
            http_method='POST',
            body_params=body_params,
        )

    def deleteVDBUpdateImmediate(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/action/updatevdb/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editVDBUpdateImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['deployAfterUpdate', 'description', 'forceOperation', 'id', 'ipAddress', 'jobHistoryUuid', 'jobName', 'name', 'scheduleType', 'type', 'user', 'version'])
    ...
    def getVDBUpdateImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getVDBUpdateImmediateList(self, params):
    ...
    def getVDBUpdateImmediateByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getVDBUpdateImmediateList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertVDBUpdateImmediate(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addVDBUpdateImmediate(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getVDBUpdateImmediateByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editVDBUpdateImmediate(params)
            else:
                raise e

    def editVDBUpdateImmediateByName(self, params):
        existing_object = self.getVDBUpdateImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editVDBUpdateImmediate(params)

    def deleteVDBUpdateImmediateByName(self, params):
        existing_object = self.getVDBUpdateImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteVDBUpdateImmediate(params)
...
def main():
    ...
    try:
        ...
        resource = VDBUpdateImmediateResource(conn)
