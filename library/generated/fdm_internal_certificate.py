...
DOCUMENTATION = """
---
module: fdm_internal_certificate
short_description: Manages InternalCertificate objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class InternalCertificateResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getInternalCertificateByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(InternalCertificateResource.getInternalCertificateList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertInternalCertificate(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return InternalCertificateResource.addInternalCertificate(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = InternalCertificateResource.getInternalCertificateByName(params)
                params = InternalCertificateResource.copy_identity_params(existing_object, params)
                return InternalCertificateResource.editInternalCertificate(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editInternalCertificateByName(params):
        existing_object = InternalCertificateResource.getInternalCertificateByName(params)
        params = InternalCertificateResource.copy_identity_params(existing_object, params)
        return InternalCertificateResource.editInternalCertificate(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteInternalCertificateByName(params):
        existing_object = InternalCertificateResource.getInternalCertificateByName(params)
        params = InternalCertificateResource.copy_identity_params(existing_object, params)
        return InternalCertificateResource.deleteInternalCertificate(params)
...
def main():
    fields = dict(
        ...
        operation=dict(choices=['addInternalCertificate', 'deleteInternalCertificate', 'editInternalCertificate', 'getInternalCertificate', 'getInternalCertificateList', 'getInternalCertificateByName', 'upsertInternalCertificate', 'editInternalCertificateByName', 'deleteInternalCertificateByName'], required=True),
    ...
    try:
        method_to_call = getattr(InternalCertificateResource, params['operation'])
