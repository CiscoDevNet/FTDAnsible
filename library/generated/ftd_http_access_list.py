...
class HTTPAccessListResource(object):
    ...
    def editHTTPAccessList(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['id', 'name', 'networkObjects', 'type', 'version'])

        return self._conn.send_request(
            url_path='/devicesettings/default/httpaccesslists/{objId}',
            http_method='PUT',
            body_params=body_params,
            path_params=path_params,
        )

    def getHTTPAccessList(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getHTTPAccessListList(self, params):
        query_params = dict_subset(params, ['filter', 'limit', 'offset', 'sort'])

        return self._conn.send_request(
            url_path='/devicesettings/default/httpaccesslists',
            http_method='GET',
            query_params=query_params,
        )

    def getHTTPAccessListByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getHTTPAccessListList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def editHTTPAccessListByName(self, params):
        existing_object = self.getHTTPAccessListByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editHTTPAccessList(params)
...
def main():
    ...
    try:
        ...
        resource = HTTPAccessListResource(conn)
