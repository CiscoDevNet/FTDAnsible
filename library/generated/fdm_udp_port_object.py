...
DOCUMENTATION = """
---
module: fdm_udp_port_object
short_description: Manages UDPPortObject objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class UDPPortObjectResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getUDPPortObjectList(params):
        query_params = dict_subset(params, ['offset', 'limit', 'sort', 'filter'])

        url = construct_url(params['hostname'], '/object/udpports', query_params=query_params)
        request_params = dict(
            headers=base_headers(params['access_token']),
            method='GET',
        )
    ...
    @staticmethod
    @retry_on_token_expiration
    def getUDPPortObjectByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(UDPPortObjectResource.getUDPPortObjectList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertUDPPortObject(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return UDPPortObjectResource.addUDPPortObject(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = UDPPortObjectResource.getUDPPortObjectByName(params)
                params = UDPPortObjectResource.copy_identity_params(existing_object, params)
                return UDPPortObjectResource.editUDPPortObject(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editUDPPortObjectByName(params):
        existing_object = UDPPortObjectResource.getUDPPortObjectByName(params)
        params = UDPPortObjectResource.copy_identity_params(existing_object, params)
        return UDPPortObjectResource.editUDPPortObject(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteUDPPortObjectByName(params):
        existing_object = UDPPortObjectResource.getUDPPortObjectByName(params)
        params = UDPPortObjectResource.copy_identity_params(existing_object, params)
        return UDPPortObjectResource.deleteUDPPortObject(params)
...
def main():
    fields = dict(
        ...
        operation=dict(choices=['addUDPPortObject', 'deleteUDPPortObject', 'editUDPPortObject', 'getUDPPortObject', 'getUDPPortObjectList', 'getUDPPortObjectByName', 'upsertUDPPortObject', 'editUDPPortObjectByName', 'deleteUDPPortObjectByName'], required=True),
    ...
    try:
        method_to_call = getattr(UDPPortObjectResource, params['operation'])
