...
DOCUMENTATION = """
---
module: fdm_external_certificate
short_description: Manages ExternalCertificate objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class ExternalCertificateResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getExternalCertificateByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(ExternalCertificateResource.getExternalCertificateList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertExternalCertificate(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return ExternalCertificateResource.addExternalCertificate(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = ExternalCertificateResource.getExternalCertificateByName(params)
                params = ExternalCertificateResource.copy_identity_params(existing_object, params)
                return ExternalCertificateResource.editExternalCertificate(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editExternalCertificateByName(params):
        existing_object = ExternalCertificateResource.getExternalCertificateByName(params)
        params = ExternalCertificateResource.copy_identity_params(existing_object, params)
        return ExternalCertificateResource.editExternalCertificate(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteExternalCertificateByName(params):
        existing_object = ExternalCertificateResource.getExternalCertificateByName(params)
        params = ExternalCertificateResource.copy_identity_params(existing_object, params)
        return ExternalCertificateResource.deleteExternalCertificate(params)
...
def main():
    fields = dict(
        ...
        operation=dict(choices=['addExternalCertificate', 'deleteExternalCertificate', 'editExternalCertificate', 'getExternalCertificate', 'getExternalCertificateList', 'getExternalCertificateByName', 'upsertExternalCertificate', 'editExternalCertificateByName', 'deleteExternalCertificateByName'], required=True),
    ...
    try:
        method_to_call = getattr(ExternalCertificateResource, params['operation'])
