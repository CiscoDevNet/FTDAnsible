...
---
module: fdm_secret
short_description: Manages Secret objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class SecretResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getSecretByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(SecretResource.getSecretList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertSecret(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return SecretResource.addSecret(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = SecretResource.getSecretByName(params)
                params = SecretResource.copy_identity_params(existing_object, params)
                return SecretResource.editSecret(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editSecretByName(params):
        existing_object = SecretResource.getSecretByName(params)
        params = SecretResource.copy_identity_params(existing_object, params)
        return SecretResource.editSecret(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteSecretByName(params):
        existing_object = SecretResource.getSecretByName(params)
        params = SecretResource.copy_identity_params(existing_object, params)
        return SecretResource.deleteSecret(params)
...
def main():
    ...
    try:
        method_to_call = getattr(SecretResource, params['operation'])
