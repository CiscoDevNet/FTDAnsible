...
class SecurityIntelligenceUpdateFeedsImmediateResource(object):
    ...
    def addSecurityIntelligenceUpdateFeedsImmediate(self, params):
        body_params = dict_subset(params, ['forceOperation', 'id', 'ipAddress', 'jobHistoryUuid', 'jobName', 'scheduleType', 'type', 'user', 'version'])

        return self._conn.send_request(
            url_path='/action/securityintelligenceupdatefeeds',
            http_method='POST',
            body_params=body_params,
        )

    def deleteSecurityIntelligenceUpdateFeedsImmediate(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/action/securityintelligenceupdatefeeds/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editSecurityIntelligenceUpdateFeedsImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['forceOperation', 'id', 'ipAddress', 'jobHistoryUuid', 'jobName', 'scheduleType', 'type', 'user', 'version'])
    ...
    def getSecurityIntelligenceUpdateFeedsImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getSecurityIntelligenceUpdateFeedsImmediateList(self, params):
    ...
    def getSecurityIntelligenceUpdateFeedsImmediateByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getSecurityIntelligenceUpdateFeedsImmediateList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertSecurityIntelligenceUpdateFeedsImmediate(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addSecurityIntelligenceUpdateFeedsImmediate(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getSecurityIntelligenceUpdateFeedsImmediateByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editSecurityIntelligenceUpdateFeedsImmediate(params)
            else:
                raise e

    def editSecurityIntelligenceUpdateFeedsImmediateByName(self, params):
        existing_object = self.getSecurityIntelligenceUpdateFeedsImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editSecurityIntelligenceUpdateFeedsImmediate(params)

    def deleteSecurityIntelligenceUpdateFeedsImmediateByName(self, params):
        existing_object = self.getSecurityIntelligenceUpdateFeedsImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteSecurityIntelligenceUpdateFeedsImmediate(params)
...
def main():
    ...
    try:
        ...
        resource = SecurityIntelligenceUpdateFeedsImmediateResource(conn)
