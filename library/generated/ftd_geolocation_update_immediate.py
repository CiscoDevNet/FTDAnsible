...
options:
  ...
  description
    description:
      - An optional String value containing description about the Geolocaiton update activity.<br>Field level constraints: length must be between 0 and 200 (inclusive). (Note: Additional constraints might exist)
  filter
    description:
  ...
  name
    description:
      - A String containing the user provided identifier associated with a scheduled activity.<br>Field level constraints: length must be between 0 and 32 (inclusive), must match pattern ^[a-zA-Z0-9][a-zA-Z0-9_+-]*$. (Note: Additional constraints might exist)
  offset
    description:
...
class GeolocationUpdateImmediateResource(object):
    ...
    def addGeolocationUpdateImmediate(self, params):
        ...
        return self._conn.send_request(
            url_path='/action/updategeolocation',
            http_method='POST',
            body_params=body_params,
        )

    def deleteGeolocationUpdateImmediate(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/action/updategeolocation/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editGeolocationUpdateImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['description', 'forceOperation', 'id', 'ipAddress', 'jobHistoryUuid', 'jobName', 'name', 'scheduleType', 'type', 'user', 'version'])
    ...
    def getGeolocationUpdateImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getGeolocationUpdateImmediateList(self, params):
    ...
    def getGeolocationUpdateImmediateByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getGeolocationUpdateImmediateList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertGeolocationUpdateImmediate(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addGeolocationUpdateImmediate(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getGeolocationUpdateImmediateByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editGeolocationUpdateImmediate(params)
            else:
                raise e

    def editGeolocationUpdateImmediateByName(self, params):
        existing_object = self.getGeolocationUpdateImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editGeolocationUpdateImmediate(params)

    def deleteGeolocationUpdateImmediateByName(self, params):
        existing_object = self.getGeolocationUpdateImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteGeolocationUpdateImmediate(params)
...
def main():
    ...
    try:
        ...
        resource = GeolocationUpdateImmediateResource(conn)
