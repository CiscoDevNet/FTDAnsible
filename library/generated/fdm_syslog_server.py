...
DOCUMENTATION = """
---
module: fdm_syslog_server
short_description: Manages SyslogServer objects on Cisco FTD devices with FDM
version_added: "2.7"
...
class SyslogServerResource(object):
    ...
    @staticmethod
    @retry_on_token_expiration
    def getSyslogServerByName(params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(SyslogServerResource.getSyslogServerList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    @staticmethod
    @retry_on_token_expiration
    def upsertSyslogServer(params):
        def is_duplicate_name_error(err):
            return err.code == 422 and "Validation failed due to a duplicate name" in str(err.read())

        try:
            return SyslogServerResource.addSyslogServer(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = SyslogServerResource.getSyslogServerByName(params)
                params = SyslogServerResource.copy_identity_params(existing_object, params)
                return SyslogServerResource.editSyslogServer(params)
            else:
                raise e

    @staticmethod
    @retry_on_token_expiration
    def editSyslogServerByName(params):
        existing_object = SyslogServerResource.getSyslogServerByName(params)
        params = SyslogServerResource.copy_identity_params(existing_object, params)
        return SyslogServerResource.editSyslogServer(params)
    ...
    @staticmethod
    @retry_on_token_expiration
    def deleteSyslogServerByName(params):
        existing_object = SyslogServerResource.getSyslogServerByName(params)
        params = SyslogServerResource.copy_identity_params(existing_object, params)
        return SyslogServerResource.deleteSyslogServer(params)
...
def main():
    ...
    try:
        method_to_call = getattr(SyslogServerResource, params['operation'])
