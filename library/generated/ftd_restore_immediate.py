...
options:
  ...
  archiveName
    description:
      - The name of the backup to be restored.<br>Field level constraints: cannot be null, length must be between 0 and 59 (inclusive), must match pattern [1-9][0-9]{13}\.NGFW_backup\.[a-zA-Z0-9_][a-zA-Z0-9_+-]*. (Note: Additional constraints might exist)
  filter
    description:
...
class RestoreImmediateResource(object):
    ...
    def addRestoreImmediate(self, params):
        ...
        return self._conn.send_request(
            url_path='/action/restore',
            http_method='POST',
            body_params=body_params,
        )

    def deleteRestoreImmediate(self, params):
        path_params = dict_subset(params, ['objId'])

        return self._conn.send_request(
            url_path='/action/restore/{objId}',
            http_method='DELETE',
            path_params=path_params,
        )

    def editRestoreImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['archiveName', 'forceOperation', 'id', 'ipAddress', 'jobHistoryUuid', 'jobName', 'preserveArchive', 'scheduleType', 'type', 'user', 'version'])
    ...
    def getRestoreImmediate(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getRestoreImmediateList(self, params):
    ...
    def getRestoreImmediateByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getRestoreImmediateList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def upsertRestoreImmediate(self, params):
        def is_duplicate_name_error(err):
            err_msg = to_text(err.read())
            return err.code == 422 and "Validation failed due to a duplicate name" in err_msg

        try:
            return self.addRestoreImmediate(params)
        except HTTPError as e:
            if is_duplicate_name_error(e):
                existing_object = self.getRestoreImmediateByName(params)
                params = copy_identity_properties(existing_object, params)
                return self.editRestoreImmediate(params)
            else:
                raise e

    def editRestoreImmediateByName(self, params):
        existing_object = self.getRestoreImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editRestoreImmediate(params)

    def deleteRestoreImmediateByName(self, params):
        existing_object = self.getRestoreImmediateByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.deleteRestoreImmediate(params)
...
def main():
    ...
    try:
        ...
        resource = RestoreImmediateResource(conn)
