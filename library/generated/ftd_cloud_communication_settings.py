...
DOCUMENTATION = """
---
module: ftd_cloud_communication_settings
short_description: Manages CloudCommunicationSettings objects on Cisco FTD devices
version_added: "2.7"
...
- name: Fetch CloudCommunicationSettings with a given name
  ftd_cloud_communication_settings:
...
class CloudCommunicationSettingsResource(object):
    ...
    def editCloudCommunicationSettings(self, params):
        path_params = dict_subset(params, ['objId'])
        body_params = dict_subset(params, ['defenseOrchestratorEnabled', 'id', 'telemetryEnabled', 'type', 'version'])

        return self._conn.send_request(
            url_path='/devicesettings/default/cloudcommunicationsettings/{objId}',
            http_method='PUT',
            body_params=body_params,
            path_params=path_params,
        )

    def getCloudCommunicationSettings(self, params):
        path_params = dict_subset(params, ['objId'])
    ...
    def getCloudCommunicationSettingsList(self, params):
        query_params = dict_subset(params, ['filter', 'limit', 'offset', 'sort'])

        return self._conn.send_request(
            url_path='/devicesettings/default/cloudcommunicationsettings',
            http_method='GET',
            query_params=query_params,
        )

    def getCloudCommunicationSettingsByName(self, params):
        search_params = params.copy()
        search_params['filter'] = 'name:%s' % params['name']
        item_generator = iterate_over_pageable_resource(self.getCloudCommunicationSettingsList, search_params)
        return next(item for item in item_generator if item['name'] == params['name'])

    def editCloudCommunicationSettingsByName(self, params):
        existing_object = self.getCloudCommunicationSettingsByName(params)
        params = copy_identity_properties(existing_object, params)
        return self.editCloudCommunicationSettings(params)


def main():
    fields = dict(
        operation=dict(type='str', choices=['editCloudCommunicationSettings', 'getCloudCommunicationSettings', 'getCloudCommunicationSettingsList', 'getCloudCommunicationSettingsByName', 'editCloudCommunicationSettingsByName'], required=True),
    ...
    try:
        ...
        resource = CloudCommunicationSettingsResource(conn)
